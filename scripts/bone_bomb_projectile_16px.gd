...
const GRAVITY := 520.0
const GROUND_CHECK_DISTANCE := 5.0
const LANDING_DELAY := 0.55
const EXPLOSION_DURATION := 0.16
const EXPLOSION_RADIUS := 34.0
const EXPLOSION_DAMAGE := 1
const REFLECTED_DAMAGE := 1
const LANDING_Y_TOLERANCE := 16.0
const REFLECTED_MIN_HORIZONTAL_SPEED := 120.0
const REFLECTED_UPWARD_VELOCITY := -120.0
...
@onready var bomb_visual: ColorRect = $BombVisual
@onready var warning_visual: ColorRect = $WarningVisual
@onready var explosion_visual: ColorRect = $ExplosionVisual
@onready var collision_shape: CollisionShape2D = $CollisionShape2D


func setup(start_velocity: Vector2, source_enemy: Node = null, target_y: Variant = null) -> void:
...
func update_flight(delta: float) -> void:
	...
	var landing_position = get_valid_landing_position(previous_position, global_position)
...
func update_explosion(delta: float) -> void:
	...
	var visual_size := lerpf(14.0, EXPLOSION_RADIUS * 2.0, progress)
...
func land() -> void:
	...
	if is_player_owned:
		bomb_visual.color = Color(0.25, 0.78, 1.0, 1)
	else:
		bomb_visual.color = Color(0.95, 0.35, 0.12, 1)
	warning_visual.show()
